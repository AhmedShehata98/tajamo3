<template>
  <div class="p-4 w-full flex-col items-start justify-start">
    ...
    <ul
      class="w-fit flex items-center justify-start gap-3 bg-zinc-200 rounded-sm mt-5 p-1.5"
    >
      <li>
        <button
          type="button"
          :class="
            activeTab === 'profile'
              ...
              : 'bg-zinc-200 text-text-primary'
          "
          class="rounded-sm px-2 p-2 transition-all duration-300"
...
watch(meData, (newUser) => {
  if (!newUser?.data) return;
  form.value.firstName = newUser?.data?.first_name;
  form.value.lastName = newUser?.data?.last_name;
  form.value.email = newUser?.data?.email;
  form.value.phone = newUser?.data?.phone;
});
